// from tokio imoort net::TcpListener - listens for incoming TCP connections + from tokio import net::TcpStream - represents a TCP stream between a client and a server + once the handshake is complete, TCP listener returns a TCP stream
...
// Using a runtime, on the other hand, involves more than just importing. It refers to setting up and utilizing a specific runtime environment designed for asynchronous programming.
#[tokio::main]
pub async fn run() -> Result<(), Box<dyn std::error::Error>>{
    let proxy: SocketAddr = ([127, 0, 0, 1], 3000).into();

    let proxy_server = TcpListener::bind(proxy).await?;

    // proxyserver.accept returns two values, the first is the client connection and the second is the client's address
...
async fn handle_client_conn(mut client_conn: TcpStream) -> Result<(), Box<dyn std::error::Error>> {
    let server: SocketAddr = ([127, 0, 0, 1], 1973).into();
